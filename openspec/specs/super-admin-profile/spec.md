...
| DTO | Fields | Rules |
|-----|--------|-------|
| UpdateSuperAdminProfileDTO | nombre | `@IsString() @MinLength(2) @MaxLength(100)`, optional |
| | apellido | `@IsString() @MinLength(2) @MaxLength(100)`, optional |
...
4. Non-existent internal SuperAdmin record (auth exists but no profile) SHALL return 404 on both GET and PATCH — should never occur if registration creates both.
