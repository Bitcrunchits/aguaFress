...
export class RpcExceptionFilter implements ExceptionFilter {
  private readonly logger = new Logger(RpcExceptionFilter.name);
    ...
    if (exception instanceof HttpException) {
      ...
      const logMessage = this.serializeForLog(errorPayload);

      if (exception.getStatus() >= HttpStatus.INTERNAL_SERVER_ERROR) {
        this.logger.error(`RPC — ${logMessage}`, exception.stack);
      } else {
        this.logger.warn(`RPC — ${logMessage}`);
      }
  ...
  private extractHttpErrorPayload(
  ...
  private extractBasePayload(
    exception: HttpException,
    response: string | object,
  ): Omit<RpcErrorPayload, 'timestamp' | 'path'> {
  ...
  private hasPatternReader(value: unknown): value is RpcContextWithPattern {
    return (
      ...
      value !== null &&
      'getPattern' in value &&
      typeof value.getPattern === 'function'
    );
  }
  ...
  private serializeForLog(body: RpcErrorPayload): string {
