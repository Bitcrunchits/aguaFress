...
import { ListLinkInvitacionDto } from './list-link-invitacion.dto';

describe('ListLinkInvitacionDto', () => {
  it('usa valores por defecto cuando se pasa objeto vacío', async () => {
    const dto = plainToInstance(ListLinkInvitacionDto, {});
  ...
  it('rechaza page como string no numérico', async () => {
    const dto = plainToInstance(ListLinkInvitacionDto, { page: 'abc' });
    const errors = await validate(dto);
  ...
  it('rechaza limit mayor a 100', async () => {
    const dto = plainToInstance(ListLinkInvitacionDto, { limit: 101 });
    const errors = await validate(dto);
    expect(errors.length).toBeGreaterThanOrEqual(1);
