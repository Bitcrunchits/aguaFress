...
// ---------------------------------------------------------------------------
// Integration tests for QrCodes: admin CRUD, vendor CRUD, guard chain
// ---------------------------------------------------------------------------
describe('QrCodes Integration', () => {
  ...
  // ────────────────────────────────────────────────────────────────────────
  // 7.1 — Vendor QR flow: create → list → deactivate
  // ────────────────────────────────────────────────────────────────────────
  describe('7.1 Vendor QR CRUD flow', () => {
    ...
    it('completa ciclo: create QR → list QR → deactivate QR', async () => {
      ...
      mockPrisma.qrCode.create.mockResolvedValue(mockCreatedQr);
      ...
      expect(createRes.body.data.qrCode).toBe('abc12345');
      expect(createRes.body.data.url).toContain('/invitar/abc12345');
      ...
      mockPrisma.qrCode.findMany.mockResolvedValue(mockQrList);
      ...
      expect(listRes.body.data[0].codigo).toBe('abc12345');

      // ── DEACTIVATE QR ──
      mockPrisma.qrCode.updateMany.mockResolvedValue({ count: 1 });

      const deactivateRes = await request(app.getHttpServer())
        .patch('/api/qr-codes/qr-1/deactivate')
        .set('Authorization', `Bearer ${vendedorToken}`)
        // deactivateInternal returns void → TransformInterceptor produces no data
        .expect(200);
    });
  });

  // ────────────────────────────────────────────────────────────────────────
  // 7.2 — Admin QR flow: list by vendedor → deactivate any QR
  // ────────────────────────────────────────────────────────────────────────
  describe('7.2 Admin QR CRUD flow', () => {
    ...
    it('admin puede listar QR de un vendedor y desactivar cualquier QR', async () => {
      const futureDate = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);
      ...
      await request(app.getHttpServer())
        .patch('/api/admin/qr-codes/qr-1/deactivate')
        .set('Authorization', `Bearer ${adminToken}`)
        // deactivateInternal returns void → only status code matters
        .expect(200);
    });
  ...
  // ────────────────────────────────────────────────────────────────────────
  // 7.3 — Auth guard chain: 401, 403 wrong role for vendor & admin
  // ────────────────────────────────────────────────────────────────────────
  describe('7.3 Auth guard chain', () => {
  ...
  // ────────────────────────────────────────────────────────────────────────
  // 7.4 — Error cases: deactivate non-existent, already inactive
  // ────────────────────────────────────────────────────────────────────────
  describe('7.4 Error cases', () => {
    let vendedorToken: string;
    ...
    it('lanza 404 al desactivar QR que no existe', async () => {
      mockPrisma.qrCode.updateMany.mockResolvedValue({ count: 0 });
      mockPrisma.qrCode.findFirst.mockResolvedValue(null);

      await request(app.getHttpServer())
        .patch('/api/qr-codes/fake-id/deactivate')
        .set('Authorization', `Bearer ${vendedorToken}`)
        .expect(404);
    });

    it('lanza 400 al desactivar QR ya inactivo', async () => {
      mockPrisma.qrCode.updateMany.mockResolvedValue({ count: 0 });
      mockPrisma.qrCode.findFirst.mockResolvedValue({
        id: 'qr-1',
        vendedor_id: 'vendor-user-1',
        codigo: 'abc12345',
        activo: false,
